			...
			<div>
				...
				<Toolbar class="p-mb-4">
					<template v-slot:left>
						<Button label="Nuevo detalle evento" icon="pi pi-plus" class="p-button-success p-mr-2" @click="openNew" />
					</template>
				...
				<DataTable ref="dt" :value="eventodetalles" v-model:selection="selectedEventodetalles" dataKey="_id" :paginator="true" :rows="10" :filters="filters"
...
export default {
	...
	async mounted() {
        await this.eventodetalleService.filtroIdEvento(this.dataEvento[0]._id).then(data =>this.eventodetalles = data.data )
	...
	methods: {
		...
		async detalleEventodetalle(evento){
			this.evento = evento
			await this.eventodetalleService.readEventodetallexId(evento._id).then(data=>this.eventosdetalles = data.data)
		...
		async saveEventodetalle() {//-------------------------------------------------------------
            ...
            if (validacion) {
                if(!this.nuevo === true) {
					...
					this.eventoService.read().then(data =>this.eventodetalles = data.data)
		...
		async deleteEventodetalle(evento) {
			...
			await this.eventoService.read().then(data=>this.eventodetalles = data.data)
